...
def mi_pairwise_tv(
    ...
    Returns:
        (tv, tv_se): Total variation distance and its standard error
    """
    ...
    # Compute SE from sample variance
    X = 0.5 * np.abs(rho1 - rho2)
    tv = float(np.mean(X))
    tv_se = float(np.sqrt(np.var(X, ddof=1) / n_samples))
    return tv, tv_se
...
def mi_scan_max_tv(n_samples: int, rng: Generator) -> Dict[str, Any]:
    ...
    for label, x1, y1, x2, y2 in pairs:
        tv, tv_se = mi_pairwise_tv(x1, y1, x2, y2, n_samples, rng)
...
def mi_mutual_information_lower_bound(n_samples: int, rng: Generator) -> Dict[str, Any]:
    """
    Compute rigorous lower bound on I(λ:X,Y) via Pinsker's inequality.
    
    This computes TV(ρ(·|x,y), ρ_mix) for each setting pair (x,y),
    where ρ_mix is the mixture over all settings, then applies Pinsker:
        I(λ:X,Y) ≥ (2/ln2) · E_{x,y}[TV(ρ(·|x,y), ρ_mix)²]
    ...
    Returns:
        Dict with MI lower bound and diagnostics
    """
    # Canonical CHSH settings
    settings = [
        (np.array([1.0, 0.0, 0.0]), np.array([1.0, 1.0, 0.0]) / np.sqrt(2)),      # (a, b)
        (np.array([1.0, 0.0, 0.0]), np.array([1.0, -1.0, 0.0]) / np.sqrt(2)),     # (a, b')
        (np.array([0.0, 1.0, 0.0]), np.array([1.0, 1.0, 0.0]) / np.sqrt(2)),      # (a', b)
        (np.array([0.0, 1.0, 0.0]), np.array([1.0, -1.0, 0.0]) / np.sqrt(2)),     # (a', b')
    ]
    
    labels = ['(a,b)', '(a,b\')', '(a\',b)', '(a\',b\')']
    
    # Sample uniformly from S² for Monte Carlo integration
    L = sample_uniform_sphere(n_samples, rng)
    
    # Compute density values for each setting pair at each sample point
    densities = []
    for a, b in settings:
        a = unit(a); b = unit(b)
        s = sector_flags(a, b, L)
        c = float(np.dot(a, b))
        ...
        eps = 1e-12
        if q_plus < eps or q_minus < eps:
            rho = np.ones(n_samples)
        else:
            rho = np.where(s, p_plus / q_plus, (1 - p_plus) / q_minus)
        densities.append(rho)
    
    densities = np.array(densities)  # shape: (4, n_samples)
    
    # Mixture density (uniform over 4 settings)
    rho_mix = np.mean(densities, axis=0)  # shape: (n_samples,)
    
    # Compute TV(ρ(·|x,y), ρ_mix) for each setting
    tvs = []
    tv_ses = []
    for i, (label, rho) in enumerate(zip(labels, densities)):
        X = 0.5 * np.abs(rho - rho_mix)
        tv = float(np.mean(X))
        tv_se = float(np.sqrt(np.var(X, ddof=1) / n_samples))
        tvs.append(tv)
        tv_ses.append(tv_se)
    
    # Average TV to mixture
    mean_tv = np.mean(tvs)
    mean_tv_squared = np.mean([tv**2 for tv in tvs])
    
    # Pinsker lower bound: I(λ:X,Y) ≥ (2/ln2) · E[TV²]
    mi_lower_bits = (2 / np.log(2)) * mean_tv_squared
    
    # SE via delta method (rough approximation)
    # Var(E[TV²]) ≈ (4·E[TV]·SE(TV))² / 4
    mean_tv_se = np.mean(tv_ses)
    mi_lower_se = (2 / np.log(2)) * 2 * mean_tv * mean_tv_se
    ...
    return {
        ...
        'tv_ses': tv_ses,
        'mean_tv': mean_tv,
        'mean_tv_squared': mean_tv_squared,
        'mi_lower_bits': mi_lower_bits,
        'mi_lower_se': mi_lower_se,
        'hall_branciard_benchmark': 0.080,
    }
...
def witness_product_data(
    ...
    for N_star in N_star_values:
        bound_product = int(np.ceil(N_star / m))
        r_max = m / N_star
        entropy_lower_bits = 2 * np.log2(2 / (1 + r_max))  # Correct entropy bound
        ...
        data.append({
            ...
            'entropy_lower_bits': entropy_lower_bits,
            ...
            'trials': n_trials,
            'pass': violations == 0,
            'entropy_pass': min_H_sum >= entropy_lower_bits - 0.1  # tolerance for float precision
        })
