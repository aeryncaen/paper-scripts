...
class TestWitnessProduct:
    ...
    def test_min_product_exceeds_bound(self, rng):
        ...
        assert all(df['min_product'] >= df['bound_product']), "min(R₁·R₂) must be >= bound"
    ...
    def test_bound_scales_with_N_star(self, rng):
        ...
        assert all(df['bound_product'].diff().dropna() > 0), "Bound should increase with N★"
    ...
    def test_min_entropy_corollary(self, rng):
        ...
        # H_∞(Π₁|W) + H_∞(Π₂|W) >= 2·log₂(2/(1+r_max))
        assert all(df['entropy_pass']), "Min-entropy sum should exceed bound"
        assert all(df['min_H_sum'] >= df['entropy_lower_bits'] - 0.1), \
            "H₁ + H₂ >= 2·log₂(2/(1+r_max)) within numerical tolerance"
